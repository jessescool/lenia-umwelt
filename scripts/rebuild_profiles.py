"""Rebuild stages 2-4 (profile → distances → neighborhood) from existing raw frames.
...
def rebuild_from_raw(raw_path: Path):
    """Run profile → distances → neighborhood from a single raw .pt file."""
    ...
    neighborhood_path = out_dir / f"{stem}_neighborhood.pt"
    torch.save(orb, neighborhood_path)
...
def main():
    ...
    print(f"Found {len(raw_files)} raw neighborhood files to rebuild")
    print()
    ...
    total = time.time() - t0
    print(f"\n{'='*60}")
    print(f"Done: rebuilt {len(raw_files)} neighborhoods (stages 2-4) in {total/60:.1f} min")
