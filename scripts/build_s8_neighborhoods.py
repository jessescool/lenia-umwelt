"""Build scale-8 neighborhoods for all creatures."""
...
def main():
    ...
    print(f"Building scale-{SCALE} neighborhoods for {len(CREATURES)} creatures")
    t0 = time.time()
    ...
    for i, code in enumerate(CREATURES):
        ...
        raw = build_neighborhood(creature, scale=SCALE, grid_size=DEFAULT_GRID,
        ...
        orb = build_neighborhood_summary(prof)
        torch.save(orb, out_dir / f"{stem}_neighborhood.pt")
        json_path = out_dir / f"{stem}_neighborhood.json"
    ...
    print(f"\nDone: {len(CREATURES)} neighborhoods in {(time.time() - t0)/60:.1f} min")
