# neighborhoods/neighborhood_batch.py — Jesse Cool (jessescool)
"""Run full neighborhood pipeline (raw → profile → distances → neighborhood) for a creature at multiple scales."""
...
def run_pipeline(code: str, scales: list[int], grid: int = 64,
    ...
    """Run full neighborhood pipeline for one creature across multiple scales."""
    ...
    for scale in scales:
        out_dir = Path(f"neighborhoods/{code}/s{scale}")
        ...
        neighborhood_path = out_dir / f"{code}_s{scale}_neighborhood.pt"
        json_path = out_dir / f"{code}_s{scale}_neighborhood.json"
        ...
        if skip_raw:
            ...
            raw_data = build_neighborhood(creature, scale=scale, grid_size=grid)
        ...
        print(f"=== {code} s{scale}: building neighborhood summary ===")
        neighborhood_data = build_neighborhood_summary(profile_data)
        torch.save(neighborhood_data, neighborhood_path)

        json_data = {k: v for k, v in neighborhood_data.items() if not isinstance(v, Tensor)}
        json_path.write_text(json.dumps(json_data, indent=2) + "\n")
        print(f"  Saved {neighborhood_path}")
        print(f"  Saved {json_path}")

    print(f"\nDone: {code} scales {scales}")
    print(f"Output: neighborhoods/{code}/")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Full neighborhood pipeline for a creature")
